...
type Context struct {
	...
	Params map[string]string //特殊加入，动态路由映射表,比如设置请求处理路径/hello/:name,来了一个请求/hello/a,则Params["name"]="a"
	// response info
	StatusCode int
...
func (c *Context) Next() {
	...
	for ; c.index < s; c.index++ {
		//以下这句代码涉及Next()的嵌套调用,但是可以
		//保证同一个中间件不会重复执行
		//可以把c.index理解为一个全局变量,如果c.handlers[c.index](c)执行
		//过程中也会执行c.Next(),则在当前for循环执行后c.index的值增加的量要大于1

		c.handlers[c.index](c) //注意该句代码执行过程中可能会再次调用Next(),涉及Next()的嵌套调用
	}
}
